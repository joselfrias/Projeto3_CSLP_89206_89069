...
def main():
    file='ducks_take_off_422_720p50.y4m'
    f=open(file, 'rb')
    ...
    while True:
        ...
        raw = f.read(int(1280*720*2))
        y = np.frombuffer(raw, dtype=np.uint8,count=1280*720)
        u= np.frombuffer(raw, dtype=np.uint8,count=int(1280*720*0.5),offset=int(1280*720))
        v= np.frombuffer(raw, dtype=np.uint8,count=int(1280*720*0.5),offset=int(1280*720*1.5))
